                                        ...
                                        Button(action: {
                                            ...
                                            if selectedFilter == filter {
                                                ...
                                                photoManager.updateFilter(for: photoId, newFilter: filter) { newImage in
                                                    ...
                                                    guard let idx = photoManager.photos.firstIndex(where: { $0.id == photoId }) else { return }
                                                    let updated = photoManager.photos[idx]
                                                    filterIntensity = updated.filterIntensity
                                                    textureIntensity = updated.textureIntensity
                                                    exposureIntensity = updated.exposureIntensity
                                                }
                                            }
                                        }) {
                                            VStack(spacing: 4) {
